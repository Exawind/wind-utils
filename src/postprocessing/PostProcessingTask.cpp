...
DEFINE_INHERITANCE_REGISTRY(PostProcessingTask);
    ...
    std::string lookup)
{
    ...
    auto it = PostProcessingTaskReg_ConstructorTable_->find(task_type);
    if (it != PostProcessingTaskReg_ConstructorTable_->end()) {
        return (it->second)(mesh, inp);
    } else {
        std::cout << "ERROR: Invalid PostProcessingTask => " << task_type << std::endl;
        std::cout << "Valid task types are: " << std::endl;
        for (const auto& t: *PostProcessingTaskReg_ConstructorTable_) {
